...
extension MathUtils {
  ...
  private static func constructAssembly(
    ...
    let orientation: VariantFragment.Orientation =
      switch orientation {
      case .horizontal:
        .horizontal
      case .vertical:
        .vertical(
          axisHeight: base.font.convertToPoints(context.constants.axisHeight.value))
      }

    return VariantFragment.from(
      parts: parts, ratio: ratio, totalAdvance: totalAdvance, base: base,
      orientation: orientation, minOverlap: minOverlap)
